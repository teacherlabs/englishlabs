(() => {
  ...
  function pathsFromHash(params) {
    ...
    const headType = headValue.toLowerCase().includes("female")
      ? "female"
      : bodyType;
    ...
    const expressionFolder =
      ["look_l", "look_r"].includes(expression) ||
      supportedExpressions.includes(expression)
        ? expression
        : "neutral";
    const paths = [`body/bodies/${bodyType}/walk.png`];
    if (legsValue.toLowerCase().startsWith("cuffed_pants")) {
      paths.push(
        `legs/cuffed/${bodyType === "male" ? "male" : "thin"}/walk.png`,
      );
    }
  ...
  function loadImage(path, recolors = {}) {
    return new Promise((resolve, reject) => {
      ...
      image.onload = () => {
        ...
        const recolorContext = recolored.getContext("2d", {
        ...
        const pixels = recolorContext.getImageData(
          0,
          0,
          image.width,
          image.height,
        ...
        for (let index = 0; index < pixels.data.length; index += 4) {
          ...
          for (const mapping of mappings) {
            const colorIndex = mapping.source.findIndex((color) => {
              ...
              return (
                parseInt(value.slice(0, 2), 16) === pixels.data[index] &&
      ...
      const assetPath = path.startsWith("spritesheets/")
        ? path
        : `spritesheets/${path}`;
